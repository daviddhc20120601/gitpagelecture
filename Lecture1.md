1. what to expect in this course: https://daviddhc20120601.github.io/academicpages.github.io/
1. basics of html css and Js and markdown
    * notepad++ [download](https://notepad-plus-plus.org/downloads/)
    * git tool [download](https://git-scm.com/downloads)
    * html [w3c](https://www.w3schools.com/html/)
    * css [w3c](https://www.w3schools.com/css/)
    * Js [w3s](https://www.w3schools.com/js/)
    * how html, css, Js work together [video](https://www.youtube.com/watch?v=7Tvao2O8gIw)
    * markdown [tutorial](https://github.com/adam-p/markdown-here/wiki/Markdown-Cheatsheet)
1. basics of gitpage
   * what is gitpage [official link](https://docs.github.com/en/pages/quickstart)
   * video [intro](https://www.youtube.com/watch?v=QyFcl_Fba-k)
   * fork from: https://daviddhc20120601.github.io/academicpages.github.io/

1. homework: think of what content you want to deliver in your website and fill the form

| Student |        content idea        |                              text |            pics |
|---------|:--------------------------:|----------------------------------:|----------------:|
| John    |        david beckham       |               what is coding:xxxx | put in a folder |
| Cathy   |     messi        |    we can enjoy our life with tv: | put in a folder |
| Smith   |    albert einstein       | physics is the rules of the world | put in a folder |
